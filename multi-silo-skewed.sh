...
# ----------------------------------------------------------------------------- compile ------------------------------------------------------------------------------------------
sudo pkill -f dbtest
mkdir -p silo-only-logs-skewed
# 100% new-order, 4 warehouses and disable fast-id
make clean && make -j dbtest MODE=perf SERIALIZE=0 PAXOS_ENABLE_CONFIG=0 STO_BATCH_CONFIG=0 SILO_SERIAL_CONFIG=0 PAXOS_ZERO_CONFIG=0 LOGGING_TO_ONLY_FILE=0 OPTIMIZED_REPLAY=0 REPLAY_FOLLOWER=0 DBTEST_PROFILER=0 SKEWED_WORKLOAD=1
...
do
  if ((skip_cgroup == 0)); then
    ...
    sudo cgexec -g cpuset:cpulimitl ./out-perf.masstree/benchmarks/dbtest --verbose --bench tpcc \
                                                                          --db-type mbta --scale-factor 4 --num-threads $trd \
                                                                          ...
                                                                          --paxos-leader-config --multi-process -P localhost -S 1000 > ./silo-only-logs-skewed/leader-$trd-1000.log 2>&1 &
  else
