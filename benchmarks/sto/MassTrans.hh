...
public:
  ...
  static void static_init() {
    Transaction::epoch_advance_callback = [] (unsigned) {
      ...
      globalepoch++;
      // alternative to set table id
      // table_name= globalepoch;
    };
  }
  ...
  static void thread_init() {
    ...
    Transaction::tinfo[TThread::id()].trans_start_callback = [] () {
        if (!mythreadinfo.ti) {  // weihshen, to avoid nullptr
            auto* ti = threadinfo::make(threadinfo::TI_PROCESS, TThread::id());
            mythreadinfo.ti = ti;
        }
  ...
  bool non_trans_write(const StringType& key,
      ...
      // we have to use lock-based cursor to ensure atomic compare-and-swap mechanism.
      ...
      if (found) {
          ...
          auto *val = (versioned_value *) versioned_value::make (new_value, 0);
          lp.value () = val;
          lp.finish (1, *ti.ti);
          fence ();
          return true;
...
private:
  ...
  bool trans_write(const StringType& key, const ValueType& value, threadinfo_type& ti = mythreadinfo) {
    ...
    if (found) {
      ...
      auto item = Sto::new_item(this, val);  // weihshen, TransItem::key_ is actuall value, TransItem::wdata_ or rdata_ is actual key in write-set and read-set, respectively
    ...
    template <bool INSERT, bool SET, typename StringType, typename ValueType>
    bool trans_write_mbta(const StringType& key, const ValueType& value,
                          bool(*compar)(const std::string& newValue,const std::string& oldValue),
                          threadinfo_type& ti = mythreadinfo) {
        // optimization to do an unlocked lookup first
        if (SET) {
...
public:
  ...
  bool transPutMbta(const KT& k, const VT& v,
                    bool(*compar)(const std::string& newValue,const std::string& oldValue),
                    threadinfo_type& ti = mythreadinfo) {
      return trans_write_mbta</*insert*/true, /*set*/true>(k, v, compar, ti);
  }
...
public:
  ...
  unsigned long long int get_identity() const override{
